...
import {
    onlyNumbersParsingToInt, retryAble,
    Stopwatch,
    STOPWATCH_FORMAT,
...
export class Lidlcrawler implements Crawler {
    ...
    async crawl(amountOfPages: number): Promise<void> {
        ...
        for (let i = 1; i <= maxPages && i <= amountOfPages; i++) {
            try {
                const loadedJsDom = await retryAble(() => goToPageReturningDom(`${this.baseUrl + this.pageUrl}${i}`));
                try {
                    this.getDataFromDocument(loadedJsDom).map(createOrUpdateProduct);
                } catch (e) {
                    console.log('Error in mapping data in crawler: ', this.type)
                }
            }
            catch (e) {
